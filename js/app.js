...
document.addEventListener('DOMContentLoaded', function () {
    ...
    // 设置最大搜索时长
    const maxSearchDurationEle = document.getElementById('maxSearchDuration')
    if (maxSearchDurationEle) {
        maxSearchDurationEle.value = localStorage.getItem('maxSearchDuration') || '3000'
        maxSearchDurationEle.addEventListener('change', function (e) {
            ...
            localStorage.setItem('maxSearchDuration', value.toString())
            showToast('已设置最大搜索时长为' + value / 1000 + '秒', 'success')
        })
    }
...
async function search() {
    ...
    try {
        ...
        const maxSearchDuration = localStorage.getItem('maxSearchDuration') || '3000'
        ...
        const searchPromises = selectedAPIs.map(async (apiId) => {
            try {
                ...
                const timeoutId = setTimeout(() => controller.abort(), parseInt(maxSearchDuration))
